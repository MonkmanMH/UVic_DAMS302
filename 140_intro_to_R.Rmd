...
```{r D1-setup}
# setup chunk with dams_numbers
dams_numbers <- c(301, 302, 404, 405, 406)
...
We can then apply a function to the object `dams_numbers`. For example, to calculate the mean, we use the function `mean()`:
...
* the minimum value of `dams_numbers`

* how many numbers there are in `dams_numbers`
...
Write the script to generate the summary of `dams_numbers`.
