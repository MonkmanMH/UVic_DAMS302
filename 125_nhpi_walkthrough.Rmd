...
For this chart theme, more than 10 lines of code gets reduced to a simple, memorable name: `dams_chart_theme`.
...
dataplot2 <- dataplot +
  ...
  scale_y_continuous(labels = comma, limits = c(80, NA)) +
  scale_colour_manual(name=NULL,
                      breaks=c("Canada", "British Columbia"),
                      labels=c("Canada", "British Columbia"), 
                      values=c("#325A80", "#CCB550")) +
  # add the DAMS chart theme elements
  dams_chart_theme

```


Just a reminder: all of the code below is what's in the `dams_chart_theme`—the last line in the code chunk above. 
...
By assigning all of this code to the object `dams_chart_theme`, we can make a change in one place, and it will carry through all of the times we need it.
